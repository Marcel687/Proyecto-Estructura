...
public class GamePanel extends JPanel {
    ...
    private ScoreManager scoreManager; // NUEVO MANAGER
    private final int TILE_SIZE = 20;
    private boolean scoreSaved = false; // Flag para guardar solo una vez

    public GamePanel(SnakeGame game) {
        this.game = game;
        this.scoreManager = new ScoreManager(); // Inicializar
            ...
            public void keyPressed(KeyEvent e) {
                if (game.isGameOver()) {
                    if (e.getKeyCode() == KeyEvent.VK_ENTER) {
                        game.reset();
                        scoreSaved = false; // Resetear flag
                        repaint();
                    }
                    return;
                }
    ...
    protected void paintComponent(Graphics g) {
        ...
        // 4. Pintar Puntaje Actual (HUD)
        g.setColor(Color.WHITE);
        g.drawString("Score: " + game.getScore(), 20, 20);

        // 5. Manejo de GAME OVER
        if (game.isGameOver()) {
            handleGameOver(g);
        }
    }
